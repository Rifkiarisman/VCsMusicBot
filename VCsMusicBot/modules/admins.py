...
@Client.on_message(filters.command("admr"))
...
async def pause(_, message: Message):
    ...
    if (chat_id not in callsmusic.active_chats) or (
        ...
        await message.reply_text("❗ Tidak Ada Musik Yang DiMainkan!")
    else:
...
async def resume(_, message: Message):
    ...
    if (chat_id not in callsmusic.active_chats) or (
        ...
        await message.reply_text("❗ Tidak Ada Yang Bisa Di Pause!")
    else:
...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Tidak Ada Musik Yang Berjalan!")
    else:
        try:
        ...
        await message.reply_text("❌ Memberhentikan Musik!")
...
async def skip(_, message: Message):
    ...
    chat_id = get_chat_id(message.chat)
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Tidak Ada Yang Bisa Di Skip!")
    else:
